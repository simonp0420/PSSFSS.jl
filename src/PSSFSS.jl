...
"""
    analyze(strata::Vector, flist, steering, outlist)
...
- `outlist`:  A matrix with 2 columns.  The first column in each row is a string
  ...
  each row is a tuple generated by the `@outputs` macro of the `Outputs` module.

"""
function analyze(strata, flist, steering, outlist; logfile="pssfss.log", gsmfile="pssfss.gsm")
    ...
    for stout in steering[1], stin in steering[2]
        ...
        for fghz in flist
            ...
            if !(angle(layers[begin].γ[1]) ≈ angle(layers[end].γ[1]) ≈ π/2)
                @info "  Skipping $(fghz) GHz due to cutoff principal modes in ambient medium"
                continue
            end
            ...
            @info "  $(t_freq) seconds total at $(fghz) GHz"
...
function calculate_jtype_gsm(layers, sheet::RWGSheet, u::Real,
    ...
    @info "      $(t_fill) seconds total matrix fill time for sheet $(is_global)"
    # Factor the matrix:
    t_temp = time()
    zmatf = factorize(zmat)
    t_factor = round(time() - t_temp, digits=tdigits)
    @info "      $(t_factor) seconds to factor matrix for sheet $(is_global)"
    t_temp = time()
    ...
    @info "      $(t_fft) seconds for basis function Fourier transforms at $(i_ft) points"
    nsolve = 0
    t_extract = 0.0
    i_extract = 0
    t_solve = 0.0
    ...
    @info "      $(round(t_extract,digits=tdigits)) seconds to extract $(i_extract) GSM entries"
    return gsm
end  
...
function calculate_mtype_gsm(layers, sheet::RWGSheet, u::Real,
    ...
    @info "      $(t_extract) seconds to extract $(i_extract) GSM entries"
    return gsm
end
...
function report_layers_sheets(strata, rwgdat, usi)
    ...
    @info "Dielectric layer information... \n"
    @info " Layer  Width  units  epsr   tandel   mur  mtandel modes  beta1x  beta1y  beta2x  beta2y"
    @info " ----- ------------- ------- ------ ------- ------ ----- ------- ------- ------- -------"
    js = 0 # sheet counter 
    jl = 0 # layer counter
    for s in strata
    ...
    @info "Sheet  Loc         Style      Rot  J/M Faces Edges Nodes Unknowns  NUFP"
    @info "-----  ---  ---------------- ----- --- ----- ----- ----- -------- ------"
